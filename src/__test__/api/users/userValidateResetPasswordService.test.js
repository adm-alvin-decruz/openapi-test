...
describe("UserResetValidatePasswordService", () => {
  ...
  describe("execute", () => {
    it("should throw an error when user not found by password_hash", async () => {
      jest
        .spyOn(userCredentialModel, "findByPasswordHash")
        .mockResolvedValue(undefined);
      await expect(
        UserValidateResetPasswordService.execute("123")
      ).rejects.toThrow(
        new Error(
          JSON.stringify({
            membership: {
    ...
    it("should throw an error when token is expired", async () => {
      ...
      jest
        .spyOn(DateUtils, "getCurrentUTCTimestamp")
        .mockReturnValueOnce("2025-01-05 07:30:30");
      await expect(
        UserValidateResetPasswordService.execute("123")
      ).rejects.toThrow(
        new Error(
    ...
    it("should return token & email when all process is passed", async () => {
      ...
      jest
        .spyOn(DateUtils, "getCurrentUTCTimestamp")
        .mockReturnValueOnce("2025-01-04 06:30:30");
