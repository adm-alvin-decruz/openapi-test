...
const { getCurrentUTCTimestamp, currentDateAddHours } = require('../../../utils/dateUtils');
...
describe('UserResetPasswordService', () => {
  ...
  describe('execute', () => {
    it('should throw an error when user not found in Cognito', async () => {
      ...
      jest.spyOn(CommonUtils, 'messageLang').mockReturnValueOnce('No record found.');
      await expect(
    ...
    it('should throw an error when can not update data in DB', async () => {
      ...
      jest
        ...
        .mockRejectedValue(new Error(JSON.stringify(CommonErrors.InternalServerError())));
      jest
    ...
    it('should return success when process reset password pass', async () => {
      ...
      jest.spyOn(CommonUtils, 'generateSaltHash').mockReturnValueOnce('ieUjtl8=');
      jest
      ...
      expect(userCredentialModel.updateByUserEmail).toBeCalledWith('test@gmail.com', {
