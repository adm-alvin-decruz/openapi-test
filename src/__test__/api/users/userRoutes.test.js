...
describe('User Routes', () => {
  ...
  describe('POST:/users/sessions - Login API', () => {
    ...
    describe('should return 400 if email is invalid', () => {
      ...
      it('email valid but domain in blacklist', async () => {
        jest.spyOn(emailDomainService, 'getCheckDomainSwitch').mockResolvedValue(true);
        jest.spyOn(emailDomainService, 'validateEmailDomain').mockReturnValue(false);
        const response = await request(app).post('/users/sessions').send({
  ...
  describe('DELETE:/users/sessions - Logout API', () => {
    ...
    it('should return 200 and logout successfully', async () => {
      ...
      jest.spyOn(emailDomainService, 'getCheckDomainSwitch').mockResolvedValue(false);
      jest.spyOn(userCredentialModel, 'findByUserEmailOrMandaiId').mockResolvedValue({
        tokens: {
  ...
  describe('GET:/users/reset-password?passwordToken=123ad - User Validate Reset Password API', () => {
    it('should return 401 if app ID is invalid', async () => {
      jest.spyOn(validationService, 'validateAppID').mockReturnValue(false);
      const response = await request(app).get('/users/reset-password?passwordToken=123ad', {
        headers: {
          'mwg-app-id': '',
        },
      });
    ...
    it('should return 200 and update successfully', async () => {
      jest.spyOn(validationService, 'validateAppID').mockReturnValue(true);
      jest.spyOn(userController, 'userValidateResetPassword').mockResolvedValue({
        membership: {
          code: 200,
