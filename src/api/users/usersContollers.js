...
async function userCreateMembershipPass(req, res) {
  ...
  if (bodyLogger.membershipPhoto && bodyLogger.membershipPhoto.bytes) {
    bodyLogger.membershipPhoto = JSON.stringify({
      bytes: maskKeyRandomly(bodyLogger.membershipPhoto.bytes),
    });
  }
  loggerService.log(
    {
      user: {
        ...
        api_header: req.headers,
        api_body: JSON.stringify(bodyLogger),
  ...
  if (!valAppID) {
    req.apiTimer.end('Route CIAM Create Membership Pass Error 401 Unauthorized', startTimer);
    return res.status(401).send(CommonErrors.UnauthorizedException(req.body.language));
  }

  const message = await UserMembershipPassValidation.validateCreateUserMembershipPass(req);
  if (message) {
    loggerService.error(
      {
        user: {
...
async function userUpdateMembershipPass(req, res) {
  ...
  const valAppID = validationService.validateAppID(req.headers);
  if (!valAppID) {
    req.apiTimer.end('Route CIAM Update Membership Pass Error 401 Unauthorized', startTimer);
    return res.status(401).send(CommonErrors.UnauthorizedException(req.body.language));
  }

  const message = await UserMembershipPassValidation.validateUpdateUserMembershipPass(req);
  if (message) {
    loggerService.error(
      {
        user: {
