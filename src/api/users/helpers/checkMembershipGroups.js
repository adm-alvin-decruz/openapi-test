  ...
  try {
    ...
    if (passkit && passkit.passType && passkit.passType.length > 1) {
      if (group === GROUP.MEMBERSHIP_PASSES) {
        ...
        return passkit.passType.every((pass) => passkitMembershipPasses.includes(pass.toLowerCase()));
      }
      //check wildpass if applicable
      return passkit.passType.every((pass) => pass === group)
    }
    const cognitoUser = userCognito ? userCognito : await cognitoService.cognitoAdminGetUserByEmail(email);
