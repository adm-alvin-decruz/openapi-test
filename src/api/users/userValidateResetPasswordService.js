...
class UserValidateResetPasswordService {
  async execute(token, lang) {
    ...
    const userInfo = await userCredentialModel.findByPasswordHash(passwordHashed);
    //can not inquiry user info inquiry by saltKey and password hashed
    if (!userInfo) {
      throw new Error(JSON.stringify(ValidateTokenErrors.ciamValidateTokenErr(lang)));
    }
    ...
    if (userInfo.tokens && userInfo.tokens.reset_token && userInfo.tokens.reset_token.reset_at) {
      throw new Error(JSON.stringify(ValidateTokenErrors.ciamValidateTokenBeingUsedErr(lang)));
    }
    //token is expire
    if (getCurrentUTCTimestamp() > userInfo.tokens.reset_token.expires_at) {
      throw new Error(JSON.stringify(ValidateTokenErrors.ciamValidateTokenExpireErr(lang)));
    }
    return {
      token,
