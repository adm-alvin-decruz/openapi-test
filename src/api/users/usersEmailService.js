...
async function createApiRequestBodyPasswordless(req, codeData) {
  ...
  const emailFrom =
    appConfig.PASSWORDLESS_EMAIL_FROM || 'Mandai Wildlife Reserve <no-reply@mandai.com>';
  const emailTemplateId =
    appConfig.PASSWORDLESS_EMAIL_TEMPLATE_ID || 'd-ce7103f8f3de4bddbba29d38631d1061';

  const appEnv = process.env.APP_ENV;
  const baseKey = `PASSWORDLESS_MAGIC_LINK_BASE_URL_${appEnv.toUpperCase()}`;
  const magicBase =
    appConfig[baseKey] || appConfig.PASSWORDLESS_MAGIC_LINK_BASE_URL || 'http://localhost:5173';
