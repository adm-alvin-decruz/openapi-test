...
const { isPasswordVersionExisted, newPasswordVersion} = require("./helpers/userPasswordVersionHelper");
...
class UserPasswordVersionService {
  async passwordValidProcessing(userId, password) {
    ...
    const passwordVersionExisted = await isPasswordVersionExisted(password, listPasswordsByConfigVersion, versionCounting);
    ...
    await newPasswordVersion(userId, passHashed, versionCounting, listPasswordsByConfigVersion, passwordInfo);
