...
class UserResetPasswordService {
  async execute(req) {
    ...
    if (disableWPResetPassword) {
      ...
      try {
        const isUserBelongWildPass = await checkUserBelongSpecificGroup(
          email,
          GROUP.WILD_PASS,
        ...
        isUserBelongWildPass &&
          (await Promise.reject(
            MembershipErrors.ciamMembershipRequestNoMPAccount(email, language),
          ));
      } catch (error) {
        await Promise.reject(MembershipErrors.ciamMembershipRequestNoMPAccount(email, language));
      }
    ...
    try {
      ...
      if (errorData.name && errorData.name === 'UserNotFoundException') {
        throw new Error(
          JSON.stringify(MembershipErrors.ciamMembershipRequestNoMPAccount(email, language)),
        );
      }
