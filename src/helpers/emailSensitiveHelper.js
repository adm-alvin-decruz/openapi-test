...
class EmailSensitiveHelper {
  ...
  static async processCaseSensitiveInputEmail(email) {
    ...
    if (
      ...
      !existsCapitalizePattern(userCredentialsInfo.email)
    ) {
      return {
    ...
    if (userSubId) {
      await userCredentialModel.updateByUserIdAndEmail(
        userCredentialsInfo.email,
        userCredentialsInfo.user_id,
        {
          user_sub_id: userSubId,
          username: normalizedEmail,
        },
      );
      await userModel.updateByUserIdAndEmail(
        userCredentialsInfo.email,
        userCredentialsInfo.user_id,
        {
    ...
    if (
      ...
      try {
        this.loggerWrapper('[CIAM-MAIN] Start Process Update Email Sensitive - Start', {
        ...
        this.loggerWrapper('[CIAM-MAIN] Start Process Update Email Sensitive - Success', {
  ...
  static async queryCognitoCaseSensitiveEmailWithRetry(email, retryTimes = 1) {
    ...
    do {
      try {
        this.loggerWrapper('[CIAM-MAIN] Start Query Cognito Email Sensitive With Retry - Start', {
          email: emailRequest,
          retryTimes: attempts,
          layer: 'EmailSensitiveHelper.queryCognitoCaseSensitiveEmailWithRetry',
        });
        //1. using AdminGetUserCommand with email user input
        response = await cognitoService.cognitoAdminGetUserByEmail(emailRequest);
        success = true;
        this.loggerWrapper('[CIAM-MAIN] Start Query Cognito Email Sensitive With Retry - Success', {
          email: emailRequest,
          retryTimes: attempts,
          layer: 'EmailSensitiveHelper.queryCognitoCaseSensitiveEmailWithRetry',
        });
  ...
  static loggerWrapper(action, loggerObj, type = 'logInfo') {
    if (type === 'error') {
      return loggerService.error({ emailSensitiveHelper: { ...loggerObj } }, {}, action);
    }

    return loggerService.log({ emailSensitiveHelper: { ...loggerObj } }, action);
  }
}
